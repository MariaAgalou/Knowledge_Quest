...
@onready var quiz = $"../Lake Quiz"
@onready var toLake: Button = $"../Lake Teleport"
...
func _on_body_entered(body):
	...
	if dialogue_triggered:
		return
	if second_pass && GameState.lake_quiz_triggered:
...
func show_dialogue1():
	...
	if dialogue_box:
		dialogue_box.text = "The hero spots the scepter fragment resting on a tiny island at the lake’s center, shimmering with mysterious power."
...
func show_dialogue2():
	...
	var panel_box = get_tree().get_first_node_in_group("PanelBox")
	if dialogue_box:
		dialogue_box.text = "Using the repaired raft, the hero reaches the island, retrieves the scepter fragment, and prepares to leave."
		panel_box.visible = true
		dialogue_box.visible = true
