...
func _on_body_entered(body):
	...
	if dialogue_triggered:
		return
	if second_pass && GameState.ancient_ruins_quiz_triggered:
...
func show_dialogue1():
	...
	if dialogue_box:
		dialogue_box.text = "He strains the stone aside, unveiling shattered pillars etched with ancient symbols, half-buried and whispering forgotten secrets."
...
func show_dialogue2():
	...
	var panel_box = get_tree().get_first_node_in_group("PanelBox")
	if dialogue_box:
		dialogue_box.text = "A hidden message reveals the scepter’s resting place nearby. He digs, uncovers the relic, and departs, the ruins falling silent once more."
		panel_box.visible = true
		dialogue_box.visible = true
