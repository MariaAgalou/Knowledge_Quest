...
func _on_body_entered(body):
	...
	if dialogue_triggered:
		return
	if second_pass && GameState.village_quiz_triggered:
...
func show_dialogue1():
	...
	if dialogue_box:
		dialogue_box.text = "The hero questions villagers, piecing together clues to uncover the hidden location of the scepter fragment."
...
func show_dialogue2():
	...
	var panel_box = get_tree().get_first_node_in_group("PanelBox")
	if dialogue_box:
		dialogue_box.text = "The hero secures the scepter part and departs the village, ready for the next challenge."
		panel_box.visible = true
		dialogue_box.visible = true
