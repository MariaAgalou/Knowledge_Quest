...
@onready var quiz = $"../Underwater Quiz"
...
func _on_body_entered(body):
	...
	if dialogue_triggered:
		return
	if second_pass && GameState.underwater_realm_quiz_triggered:
...
func show_dialogue1():
	...
	if dialogue_box:
		dialogue_box.text = "To reach the sunken ship, the hero gathers pearls, shells, and magical algae for the sea queen, earning passage to the rumored scepter fragment."
...
func show_dialogue2():
	...
	var panel_box = get_tree().get_first_node_in_group("PanelBox")
	if dialogue_box:
		dialogue_box.text = "The hero secures the scepter fragment, but oxygen fades fast—he must escape the sunken ship before it’s too late."
		panel_box.visible = true
		dialogue_box.visible = true
