...
func _on_mouse_entered():
	glow.visible = true
	level.text = "Level 13: The Maze"
	level.visible = true
	Input.set_default_cursor_shape(Input.CURSOR_POINTING_HAND)
	if GameState.underwater_realm_level_completed:
		text.text = "Click to enter!"
		text.visible = true
	else:
		text.text = "Complete level 12: The Underwater Realm first!"
		text.visible = true
...
func _input_event(_viewport, event, _shape_idx):
	if GameState.underwater_realm_level_completed:
		if event is InputEventMouseButton and event.pressed:
			...
			SceneManager.change_scene("res://scenes/insidemaze.tscn")
			Input.set_default_cursor_shape(Input.CURSOR_ARROW)
