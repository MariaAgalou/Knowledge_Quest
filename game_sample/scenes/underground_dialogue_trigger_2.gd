...
@onready var quiz = $"../Underground Quiz"
...
func _on_body_entered(body):
	...
	if dialogue_triggered:
		return
	if second_pass && GameState.underground_dungeon_quiz_triggered:
...
func show_dialogue1():
	...
	if dialogue_box:
		dialogue_box.text = "The hero steps inside, torchlight revealing worn tiles etched with ancient symbols whispering forgotten warnings."
...
func show_dialogue2():
	...
	var panel_box = get_tree().get_first_node_in_group("PanelBox")
	if dialogue_box:
		dialogue_box.text = "The pattern clicks, a door opens to a vast hall of sarcophagi. The hero finds the scepter, grabs it, and flees the collapsing dungeon."
		panel_box.visible = true
		dialogue_box.visible = true
