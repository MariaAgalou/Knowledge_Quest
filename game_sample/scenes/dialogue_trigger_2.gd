...
func _on_body_entered(body):
	...
	if dialogue_triggered:
		return
	if second_pass && GameState.castle_quiz_triggered:
...
func show_dialogue1():
	...
	if dialogue_box:
		dialogue_box.text = "The door creaks open; torchlight glints off a lone chest, silent and waiting, promising glory or ruin."
...
func show_dialogue2():
	...
	var panel_box = get_tree().get_first_node_in_group("PanelBox")
	if dialogue_box:
		dialogue_box.text = "Inside rests the scepter fragment; clutching it tight, the hero turns away, destiny urging him onward."
		panel_box.visible = true
		dialogue_box.visible = true
