...
func _on_body_entered(body):
	...
	if dialogue_triggered:
		return
	if second_pass && GameState.mountains_quiz_triggered:
...
func show_dialogue1():
	...
	if dialogue_box:
		dialogue_box.text = "The mountains loom vast and endless, promising a long, arduous search that will test the hero’s patience and resolve."
...
func show_dialogue2():
	...
	var panel_box = get_tree().get_first_node_in_group("PanelBox")
	if dialogue_box:
		dialogue_box.text = "Spotting the scepter piece nearby, the hero retrieves it swiftly and departs before danger arrives."
		panel_box.visible = true
		dialogue_box.visible = true
