...
func _on_body_entered(body):
	...
	if dialogue_triggered:
		return
	if second_pass && GameState.river_quiz_triggered:
...
func show_dialogue1():
	...
	if dialogue_box:
		dialogue_box.text = "Spotting floating debris, the hero gathers tall river grass and sturdy reeds, weaving a net to retrieve the drifting items."
...
func show_dialogue2():
	...
	var panel_box = get_tree().get_first_node_in_group("PanelBox")
	if dialogue_box:
		dialogue_box.text = "The hero nets a river-drifting relic, revealed as a scepter fragment, then hastily departs before new threats emerge."
		panel_box.visible = true
		dialogue_box.visible = true
