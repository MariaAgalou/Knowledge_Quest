...
func _on_mouse_entered():
	glow.visible = true
	level.text = "Level 10: The Abandoned Tower"
	level.visible = true
	Input.set_default_cursor_shape(Input.CURSOR_POINTING_HAND)
	if GameState.ancient_ruins_level_completed:
		text.text = "Click to enter!"
		text.visible = true
	else:
		text.text = "Complete level 9: The Ruins first!"
		text.visible = true
...
func _input_event(_viewport, event, _shape_idx):
	if GameState.ancient_ruins_level_completed:
		if event is InputEventMouseButton and event.pressed:
			...
			SceneManager.change_scene("res://scenes/insidetower.tscn")
			Input.set_default_cursor_shape(Input.CURSOR_ARROW)
