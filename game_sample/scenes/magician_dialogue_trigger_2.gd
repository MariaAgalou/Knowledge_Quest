...
@onready var dialogue: Area2D = $"../MagicianDialogueTrigger"
var dialogue_1 := false
...
func _on_body_entered(body):
	...
	if dialogue_triggered:
		return
	if dialogue.dialogue_triggered_2:
		if second_pass && GameState.magician_lair_quiz_triggered:
...
func show_dialogue1():
	...
	if dialogue_box:
		dialogue_box.text = "The hero explores dusty shelves of spellbooks and strange ingredients, experimenting with bubbling cauldrons, mixing powders and herbs to brew a powerful magical filter."
...
func show_dialogue2():
	...
	var panel_box = get_tree().get_first_node_in_group("PanelBox")
	if dialogue_box:
		dialogue_box.text = "With Enigmor defeated, the hero uses the magic filter to restore the scepter, then departs quietly, leaving the ruins cleansed and the magic at peace."
		panel_box.visible = true
		dialogue_box.visible = true
