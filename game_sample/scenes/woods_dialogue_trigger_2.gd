...
func _on_body_entered(body):
	...
	if dialogue_triggered:
		return
	if second_pass && GameState.woods_quiz_triggered:
...
func show_dialogue1():
	...
	if dialogue_box:
		dialogue_box.text = "The hero kneels, studying fresh tracks in the dirt, then rises and follows them into the shadowed forest."
...
func show_dialogue2():
	...
	var panel_box = get_tree().get_first_node_in_group("PanelBox")
	if dialogue_box:
		dialogue_box.text = "The hero secures the scepter fragment, then descends the tree, ready to continue the journey."
		panel_box.visible = true
		dialogue_box.visible = true
