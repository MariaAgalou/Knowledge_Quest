...
func _on_mouse_entered():
	glow.visible = true
	level.text = "Level 5: The Orchard"
	level.visible = true
	level2.text = "Level 5: The Orchard"
	level2.visible = true
	if GameState.cave_level_completed:
		...
		text.text = "Complete level 4: The Cave first!"
		text2.text = "Complete level 4: The Cave first!"
		text.visible = true
		text2.visible = true
...
func _input_event(_viewport, event, _shape_idx):
	if GameState.cave_level_completed:
		if event is InputEventMouseButton and event.pressed:
			...
			SceneManager.change_scene("res://scenes/insideorchard.tscn")
			Input.set_default_cursor_shape(Input.CURSOR_ARROW)
