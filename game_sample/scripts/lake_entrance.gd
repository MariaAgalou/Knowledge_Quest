...
func _on_mouse_entered():
	glow.visible = true
	level.text = "Level 7: The Lake"
	level.visible = true
	level2.text = "Level 7: The Lake"
	level2.visible = true
	Input.set_default_cursor_shape(Input.CURSOR_POINTING_HAND)
	if GameState.river_level_completed:
		text.text = "Click to enter!"
		text.visible = true
		text2.text = "Click to enter!"
		text2.visible = true
	else:
		text.text = "Complete level 6: The River first!"
		text2.text = "Complete level 6: The River first!"
		text.visible = true
		text2.visible = true
...
func _input_event(_viewport, event, _shape_idx):
	if GameState.river_level_completed:
		if event is InputEventMouseButton and event.pressed:
			...
			SceneManager.change_scene("res://scenes/insidelake.tscn")
			Input.set_default_cursor_shape(Input.CURSOR_ARROW)
